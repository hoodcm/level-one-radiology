...
- **Signals** (`--color-signal-*`) — functional, not decorative; each one carries meaning: **red** = critical / trauma, **cyan** = links / interactive, **yellow / gold** = caution (and the primary CTA, via `--color-primary`), **violet** = code / technical, **orange** = warning. Use a signal only when it means its thing.
